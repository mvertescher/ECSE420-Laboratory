...
void p2p_cons_run(struct GEmpi *param);
void p2p_cons_get_rows(struct GEmpi *param);
void p2p_cons_distribute_rows(struct GEmpi *param);
void p2p_cons_eliminate(struct GEmpi *param);
void p2p_cons_reduce_rows(struct GEmpi *param);
...
void p2p_cons_run(struct GEmpi *param)
{
  ...
  double t1 = timer();
  p2p_cons_get_rows(param);
  p2p_cons_eliminate(param);
  p2p_cons_reduce_rows(param);
  param->t_exec = timer() - t1;
  param->t_proc = param->t_exec - param->t_comm;
  p2p_cons_report_result(param);
} 
...
void p2p_cons_get_rows(struct GEmpi *param) 
{
  ...
  if (param->rank == 0) {
    p2p_cons_distribute_rows(param);
  } else {
  	MPI_Status status;
  	int r;
    double t1;
...
void p2p_cons_eliminate(struct GEmpi *param) {
  ...
  // For every row owned
  for (row = 0; row < blocking_factor; row++) {
    // For every higher rank
    for (cur_rank = rank + 1; cur_rank < size; cur_rank++) {
      // Send the current row to the proc
      t1 = timer();
...
void p2p_cons_report_result(struct GEmpi *param) 
{
  ...
  char file_name[50];
  sprintf(file_name,"./results/p2p-cons-%i-%i.dat",param->N,param->size);
