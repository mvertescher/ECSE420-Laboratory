...
void p2p_cycl_run(struct GEmpi *param);
void p2p_cycl_get_rows(struct GEmpi *param);
void p2p_cycl_distribute_rows(struct GEmpi *param);
void p2p_cycl_eliminate(struct GEmpi *param);
void p2p_cycl_reduce_rows(struct GEmpi *param);
...
void p2p_cycl_run(struct GEmpi *param)
{
  ...
  double t1 = timer();
  p2p_cycl_get_rows(param);
  p2p_cycl_eliminate(param);
  p2p_cycl_reduce_rows(param);
  param->t_exec = timer() - t1;
  param->t_proc = param->t_exec - param->t_comm;
  p2p_cycl_report_result(param);
} 
...
void p2p_cycl_get_rows(struct GEmpi *param) 
{
  ...
  if (param->rank == 0) {
    p2p_cycl_distribute_rows(param);
  } else {
  	MPI_Status status;
  	int r;
    double t1;
...
void p2p_cycl_eliminate(struct GEmpi *param) {
  ...
  for (pivot = 0; pivot < N; pivot++) {
    ...
    if (cur_rank == rank) {
      // Send the current row to all processes that require it
      // If the pivot plus the num_procs is greater than N
      // Only send to higher ranks 
      if (pivot + size > N) {
        r = cur_rank + 1;
      } else {
      ...
      for (; r < size; r++) {
        // Send the current row to the proc except self
        if (r != rank) {
          t1 = timer();
...
void p2p_cycl_report_result(struct GEmpi *param) 
{
  ...
  char file_name[50];
  sprintf(file_name,"./results/p2p-cycl-%i-%i.dat",param->N,param->size);
